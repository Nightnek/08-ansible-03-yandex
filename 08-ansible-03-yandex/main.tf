resource "yandex_vpc_network" "swarm" {
  name = var.vpc_name
}
resource "yandex_vpc_subnet" "swarm" {
  name           = var.vpc_name
  zone           = var.default_zone
  network_id     = yandex_vpc_network.swarm.id
...
resource "yandex_compute_instance" "clickhouse" {
  ...
  network_interface {
    subnet_id = yandex_vpc_subnet.swarm.id
    nat       = var.vm_nat
  }

  metadata = {
    serial-port-enable = 1
    ssh-keys = "ubuntu:ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIOmq72t4OpWQw3bQC/bMZ4g+pexLbMxHRiZsUuQ+hZGb g-l-s@Nightnek_laptop"
  }
}
